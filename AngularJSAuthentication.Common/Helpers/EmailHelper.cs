...
namespace AngularJSAuthentication.Common.Helpers
{
    ...
    {
        public static bool SendMail(String From, String To, String BCC, String Subject, String Message, string File)
        {
            bool isSent = false;
            ...
            try
            {
                ...
                //{
                //    From = EmailConstant.InvoiceFrom;
                //}
                //if (string.IsNullOrEmpty(BCC))
                //{
                //    BCC = EmailConstant.InvoiceBCC;
                //}
                ...
                mMailMessage.Body = Message; //Message.ToString();
                mMailMessage.Priority = MailPriority.Normal;
                if (File.Length != 0)
                {
                    try
                    {
                        string[] AttObjstr = File.Split(',');
                        for (int i = 0; i < AttObjstr.Length; i++)
                        {

                            if (System.IO.File.Exists(AttObjstr[i].ToString()))
                            {
                                string mmtype = MIMEAssistant.GetMIMEType(AttObjstr[i].ToString());
                                System.Net.Mail.Attachment attachment = new System.Net.Mail.Attachment(AttObjstr[i].ToString(), mmtype);
