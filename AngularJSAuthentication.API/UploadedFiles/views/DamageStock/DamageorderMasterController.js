    ...
    function ($scope, WarehouseService, DamageOrderMasterService, DamageOrderDetailsService, $http, ngAuthSettings, $filter, ngTableParams, $modal) {
        ...
        $scope.searchdata = function (data) {
            ...
            if (!$('#dat').val() && $scope.srch == "") {
                ...
                $scope.srch = { orderId: 0, LScode: "", shopName: "", mobile: "", status: '' }
            }
            else if ($scope.srch != "" && !$('#dat').val()) {
                start = null;
                end = null;
            ...
            var url = serviceBase + "api/SearchOrder?start=" + start + "&end=" + end + "&OrderId=" + $scope.srch.orderId + "&LScode=" + $scope.srch.LScode + "&ShopName=" + $scope.srch.shopName + "&Mobile=" + $scope.srch.mobile + "&status=" + stts;
            $http.get(url).success(function (response) {
        ...
        $scope.exportData = function (data) {
            ...
            var url = serviceBase + "api/SearchOrder?type=export&start=" + start + "&end=" + end + "&OrderId=" + $scope.srch.orderId + "&LScode=" + $scope.srch.LScode + "&ShopName=" + $scope.srch.shopName + "&Mobile=" + $scope.srch.mobile + "&status=" + $scope.srch.status;
            $http.get(url).success(function (response) {
                ...
                if ($scope.OrderByDate.length <= 0) {
                    ...
                    for (var i = 0; i < $scope.OrderByDate.length; i++) {
                        ...
                        var LScode = $scope.OrderByDate[i].LScode;
                        ...
                        for (var j = 0; j < orderDetails.length; j++) {
                            var tts = {
                                OrderId: '', LScode: '', ShopName: '', Mobile: '', OrderBy: '', RetailerName: '', RetailerID: '', Warehouse: '', ClusterName: '', Deliverydate: '', CompanyId: '', BillingAddress: '', Date: '',
                    ...
                    alasql('SELECT OrderId,LScode,ShopName,RetailerName,Mobile,ItemID,ItemName,CategoryName,BrandName,SKU,Warehouse,ClusterName,Date,OrderBy,Excecutive,MRP,UnitPrice,MOQPrice,Quantity,Discount,DiscountPercentage,Tax,TaxPercentage,TotalAmt,deliveryCharge,Status,ReasonCancle,comments INTO XLSX("OrderDetails.xlsx",{headers:true}) FROM ?', [$scope.NewExportData]);
                }
            });
        };
        //-------------------------------------------------------------------------//
        ...
        $scope.exportDataAll = function (data) {
            ...
            var url = serviceBase + "api/SearchOrder/all?type=export&start=" + start + "&end=" + end + "&OrderId=" + $scope.srch.orderId + "&LScode=" + $scope.srch.LScode + "&ShopName=" + $scope.srch.shopName + "&Mobile=" + $scope.srch.mobile + "&status=" + $scope.srch.status;
            $http.get(url).success(function (response) {
                $scope.OrderByDate = response;
                console.log("export");
                if ($scope.OrderByDate.length <= 0) {
...
app.controller("ModalInstanceCtrldeleteOrder", ["$scope", '$http', 'DamageOrderMasterService', "$modalInstance", 'ngAuthSettings', 'order',
    function ($scope, $http, DamageOrderMasterService, $modalInstance, ngAuthSettings, myData) {
